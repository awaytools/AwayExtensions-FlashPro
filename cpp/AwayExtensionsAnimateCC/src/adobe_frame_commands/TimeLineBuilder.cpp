...
namespace AwayJS
{
    ...
    FCM::Result TimelineBuilder::AddMovieClip(FCM::U_Int32 objectId, MOVIE_CLIP_INFO* pMovieClipInfo, DOM::FrameElement::PIMovieClip pMovieClip)
    {
        ...
        if(pButton.m_Ptr)
        {
            ...
            if(trackMode == DOM::FrameElement::TRACK_AS_BUTTON)
            {
				...
				std::string res_id = FILES::int_to_string(pMovieClipInfo->resourceId);
				BLOCKS::MovieClip* thisBlock = reinterpret_cast<BLOCKS::MovieClip*>(this->m_pTimelineWriter->awd->get_project()->get_block_by_external_id_shared(res_id));
				if(thisBlock==NULL){
	...
	void TimelineBuilder::Init(AnimateToAWDEncoder* awd)
    {
    ...
    void TimelineBuilderFactory::Init(AnimateToAWDEncoder* p_flash_to_awd)
    {
