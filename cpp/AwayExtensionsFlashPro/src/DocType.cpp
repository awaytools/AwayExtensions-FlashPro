...
namespace AwayJS
{
    ...
    void FeatureMatrix::Init(FCM::PIFCMCallback pCallback)
    {
        ...
        FCM::StringRep16 outputString = Utils::ToString16(std::string("\nThe feature settings for the AwayJS document type is read from "),GetCallback());
        outputConsoleService->Trace(outputString);
        outputConsoleService->Trace(path);

        FCM::AutoPtr<FCM::IFCMCalloc> pCalloc = AwayJS::Utils::GetCallocService(pCallback);
...

        {
...

            {
...

                {
                    ...
                    std::string str_name_uni = "AwayJS";
                    res = pCategory->Add(
                    ...
                    std::string str_desc = "The AwayJS document can be used to author content for AwayJS runtime";
                    res = pCategory->Add(
