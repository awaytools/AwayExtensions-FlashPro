...
FCM::Result FlashToAWDEncoder::ExportFillStyle(std::string& this_lookup_str, FCM::PIFCMUnknown pFillStyle)
{
    ...
    if (pSolidFillStyle)
    {
        res = ExportSolidFillStyle(this_lookup_str, pSolidFillStyle);
		...
		if (pGradientFillStyle)
		{
			...
			{
				res = ExportRadialGradientFillStyle(this_lookup_str, pGradientFillStyle);
				ASSERT(FCM_SUCCESS_CODE(res));
				return res;
			}
			else if (FCM::AutoPtr<DOM::Utils::ILinearColorGradient>(pGrad))
			{
				res = ExportLinearGradientFillStyle(this_lookup_str, pGradientFillStyle);
				ASSERT(FCM_SUCCESS_CODE(res));
				return res;
			}
		}
			...
			if (pBitmapFillStyle)
			{
				res = ExportBitmapFillStyle(this_lookup_str, pBitmapFillStyle);
				ASSERT(FCM_SUCCESS_CODE(res));
				return res;
			}
		}
...
FCM::Result FlashToAWDEncoder::ExportSolidFillStyle(std::string& this_lookup_str, DOM::FillStyle::ISolidFillStyle* pSolidFillStyle)
{
    FCM::Result res;
	...
	this_lookup_str+=sstream_color.str()+"#";
...
FCM::Result FlashToAWDEncoder::ExportRadialGradientFillStyle(std::string& this_lookup_str,DOM::FillStyle::IGradientFillStyle* pGradientFillStyle)
{
  ...
  //  ASSERT(FCM_SUCCESS_CODE(res));
    ...
    for ( i = 0; i < nColors; i++)
    {
		...
		this_lookup_str+=std::to_string(newColor.x)+","+std::to_string(newColor.y)+","+std::to_string(newColor.z)+","+std::to_string(newColor.w)+",";
		this_lookup_str+=std::to_string(point.pos)+"/";
    }
	this_lookup_str+="#";
...
FCM::Result FlashToAWDEncoder::ExportLinearGradientFillStyle(std::string& this_lookup_str, DOM::FillStyle::IGradientFillStyle* pGradientFillStyle)
{    
	DOM::FillStyle::GradientSpread spread;	
    FCM::AutoPtr<FCM::IFCMUnknown> pGrad;
    FCM::AutoPtr<DOM::FillStyle::IGradientFillStyle> gradientFill = pGradientFillStyle;
...
FCM::Result FlashToAWDEncoder::ExportBitmapFillStyle(std::string& this_lookup_str, DOM::FillStyle::IBitmapFillStyle* pBitmapFillStyle)
{
	...
	this_lookup_str+=new_bitmaptexture->get_url()+"#";
    
	std::string matname = "mat_"+new_bitmaptexture->get_name();//+std::to_string(this->grafik_cnt++);
	// get the material-block for this color (create if does not exist)
