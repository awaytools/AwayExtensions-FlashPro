    ...
    case DOMNode::DOCUMENT_TYPE_NODE:  // Not to be shown to Filter
        {
            ...
            (
                *fFormatter << gStartDoctype << nodeName;
                ...
                const XMLCh  *iden = doctype->getPublicId();
                if (iden && *iden)
                {
                    *fFormatter << chSpace << gPublic << iden << chDoubleQuote;

                    iden = doctype->getSystemId();
                    if (iden && *iden)
                    {
                        *fFormatter << chSpace << chDoubleQuote << iden << chDoubleQuote;
                    }
                    else
                    {
                        //
                ...
                else
                {
                    ...
                    if (iden && *iden)
                    {
                        *fFormatter << chSpace << gSystem << iden << chDoubleQuote;
                    }
                }

                iden = doctype->getInternalSubset();
                if (iden && *iden)
                {
                    *fFormatter << chSpace << chOpenSquare << iden << chCloseSquare;
                }
    ...
    case DOMNode::ENTITY_NODE:  // Not to be shown to Filter
        {
            ...
            *fFormatter << gStartEntity    << nodeName;
            ...
            const XMLCh * iden = ((const DOMEntity*)nodeToWrite)->getPublicId();
            if (iden)
                *fFormatter << gPublic << iden << chDoubleQuote;

            iden = ((const DOMEntity*)nodeToWrite)->getSystemId();
            if (iden)
                *fFormatter << gSystem << iden << chDoubleQuote;

            iden = ((const DOMEntity*)nodeToWrite)->getNotationName();
            if (iden)
                *fFormatter << gNotation << iden << chDoubleQuote;
