...
void DOMNodeIDMap::add(DOMAttr *attr)
{
	...
	//
	const XMLCh *iden=attr->getValue();
        XMLSize_t initalHash = XMLString::hash(iden, fSize-1);
	initalHash++;
...
DOMAttr *DOMNodeIDMap::find(const XMLCh *iden)
{
    //
    ...
    while ((tableSlot= fTable[currentHash])!=0)
	{
        if ((tableSlot != (DOMAttr *)-1) && XMLString::equals(tableSlot->getValue(), iden))
            return tableSlot;
