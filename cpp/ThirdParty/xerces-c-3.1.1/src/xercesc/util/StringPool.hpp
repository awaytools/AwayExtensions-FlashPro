...
public :
    ...
    virtual bool exists(const unsigned int iden) const;
    virtual void flushAll();
    virtual unsigned int getId(const XMLCh* const toFind) const;
    virtual const XMLCh* getValueForId(const unsigned int iden) const;
...
inline bool XMLStringPool::exists(const unsigned int iden) const
{
    return (iden > 0 && (iden < fCurId));
}

inline const XMLCh* XMLStringPool::getValueForId(const unsigned int iden) const
{
    if (!iden || (iden >= fCurId))
