...
{
    unsigned int iden = fConstPool->getId(newString);
    if(iden)
        return iden;
    ...
    iden = XMLStringPool::addOrFind(newString);
    return iden+constCount;
}
...
bool XMLSynchronizedStringPool::exists(const unsigned int iden) const
{
    if (!iden)
        return false;
...
const XMLCh* XMLSynchronizedStringPool::getValueForId(const unsigned int iden) const
{
    if (iden <= fConstPool->getStringCount())
        return fConstPool->getValueForId(iden);

    unsigned int constCount = fConstPool->getStringCount();
    XMLMutexLock lockInit(&const_cast<XMLSynchronizedStringPool*>(this)->fMutex);
    return XMLStringPool::getValueForId(iden-constCount);
}
